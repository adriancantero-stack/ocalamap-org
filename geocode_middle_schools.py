...
def geocode_address(address):
    ...
    params = {
        ...
        "format": "json",
        "limit": 1,
        "email": "antigravity@google.com" # Polite usage
    }
    headers = {
...
def main():
    with open('user_schools_middle_parsed.json', 'r') as f:
        schools = json.load(f)
    ...
    for i, school in enumerate(schools):
        ...
        # Generate ID
        school['id'] = school['name'].lower().replace(' ', '-').replace("'", "").replace("’", "") + "-middle"
        
        geocoded_schools.append(school)
        time.sleep(1.1) # Rate limit
        
    with open('user_schools_middle_geocoded.json', 'w') as f:
